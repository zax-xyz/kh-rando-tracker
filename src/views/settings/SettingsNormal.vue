...
div
  ...
  .row.grid
    input(placeholder="5" v-model="settings.columns")
    p.name Number of grid columns

  .row.grid
    input(placeholder="100px" v-model="settings.size")
    p.name Item size

  .row.grid
    input(placeholder="7.5px" v-model="settings.padding")
    div
      p.name Item padding
      BaseTooltip Pre-2.2 versions used 7.5px

  .row.grid
    input(placeholder="150" v-model.number="settings.longPressDelay")
    div
      p.name Long Press Delay
      BaseTooltip Delay in milliseconds for long-press on mobile to trigger a right-click

  .row.grid
    button(:class="{ alt: drag }" style="grid-row: 1" @click="toggleDrag") Toggle Rearrange
