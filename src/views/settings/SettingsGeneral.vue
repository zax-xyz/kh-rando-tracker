...
div
  .row.grid
    input(placeholder="#212121" v-model="settings.bgColor")
    div
  ...
  .row.grid
    input.long(placeholder="https://..." v-model="settings.bgImg")
    div
  ...
  .row.grid
    button(@click="save" style="grid-row: 1; margin-left: auto") Export Settings
    div(style="grid-row: 1")
      button(@click="load") Import Settings
      input(ref="fileLoader" type="file" accept=".json" style="display: none" @change="onChange")
...
export default class SettingsGeneral extends Vue {
  ...
  onChange(event: Event): void {
    ...
    reader.onload = () => {
      this.$store.commit("settings/setSettings", JSON.parse(reader.result as string));
    };
