...
try {
  bgInputElem.value = localStorage.bg ?? null;
} catch {}
...
try {
  iconOrder.value = localStorage.order ?? null;
} catch {}
iconOrder.oninput = (event) => {
  const order = event.target.value.trim();
  grid.innerHTML = "";

  if (order) order.split(" ").forEach((i) => grid.appendChild(icons[i - 1]));
  else icons.forEach((icon) => grid.appendChild(icon));
...
try {
  iconRemove.value = localStorage.remove ?? null;
} catch {}
...
icon_style.onchange = (ev) => {
  ...
  try {
    localStorage.iconStyle = ev.target.value;
  } catch {}
};
